  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-12">
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        {brand && (
          <Link href={`/brands/${brand.slug}`} className="text-muted hover:underline">
            {brand.name}
          </Link>
        )}
      ...
      <h1 className="mt-1 font-heading text-3xl font-semibold text-foreground">{product.name}</h1>
      {product.released_at && (
      ...
      <section className="mt-10">
        <h2 className="text-sm font-medium uppercase tracking-wide text-muted">Articles</h2>
        {articles?.length ? (
          ...
          <p className="mt-3 text-sm text-muted">Aucun article pour ce produit.</p>
        )}
      </section>
    </main>
  );
